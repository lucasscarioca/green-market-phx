defmodule GreenMarketWeb.AdminAuthTest do
  ...
  @remember_me_cookie "_green_market_web_admin_remember_me"

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, GreenMarketWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_admin/3" do
    test "stores the admin token in the session", %{conn: conn, admin: admin} do
      ...
      assert Admin_accounts.get_admin_by_session_token(token)
    end
  ...
  describe "logout_admin/1" do
    test "erases session and cookies", %{conn: conn, admin: admin} do
      admin_token = Admin_accounts.generate_admin_session_token(admin)
      ...
      assert redirected_to(conn) == "/"
      refute Admin_accounts.get_admin_by_session_token(admin_token)
    end

    test "broadcasts to the given live_socket_id", %{conn: conn} do
      live_socket_id = "admins_sessions:abcdef-token"
      GreenMarketWeb.Endpoint.subscribe(live_socket_id)
