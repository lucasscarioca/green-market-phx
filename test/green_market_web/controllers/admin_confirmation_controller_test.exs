defmodule GreenMarketWeb.AdminConfirmationControllerTest do
  use GreenMarketWeb.ConnCase, async: true

  alias GreenMarket.Admin_accounts
  alias GreenMarket.Repo
  ...
  describe "POST /admins/confirm" do
    ...
    test "sends a new confirmation token", %{conn: conn, admin: admin} do
      ...
      assert Repo.get_by!(Admin_accounts.AdminToken, admin_id: admin.id).context == "confirm"
    end

    test "does not send confirmation token if Admin is confirmed", %{conn: conn, admin: admin} do
      Repo.update!(Admin_accounts.Admin.confirm_changeset(admin))
      ...
      assert redirected_to(conn) == "/"
      assert get_flash(conn, :info) =~ "If your email is in our system"
      refute Repo.get_by(Admin_accounts.AdminToken, admin_id: admin.id)
    end
    ...
    test "does not send confirmation token if email is invalid", %{conn: conn} do
      ...
      assert redirected_to(conn) == "/"
      assert get_flash(conn, :info) =~ "If your email is in our system"
      assert Repo.all(Admin_accounts.AdminToken) == []
    end
  end
  ...
  describe "POST /admins/confirm/:token" do
    test "confirms the given token once", %{conn: conn, admin: admin} do
      ...
      assert Admin_accounts.get_admin!(admin.id).confirmed_at
    ...
    test "does not confirm email with invalid token", %{conn: conn, admin: admin} do
      ...
      assert redirected_to(conn) == "/"
      assert get_flash(conn, :error) =~ "Admin confirmation link is invalid or it has expired"
      refute Admin_accounts.get_admin!(admin.id).confirmed_at
    end
  end
end
